...
## Promts
Ich möchte für alle Aufgaben und Lösungen im Repo ein Web-Interface implementieren (Online Kurs). Damit die Schüler Schritt für Schritt die Aufgaben in Form eines Kurses online lösen können, sich testen und kontrollieren können. Ich schildere jetzt kurz wie ich mir das Layout der Web-Oberfläche und Lösung vorstelle und Du sagst mir ob eine derartige Vorgehensweise sinnvoll und möglich ist. Und ggf. welche Schritte ich gehen musss um das umzusetzten. Aus den Online Kursen die ich kenne gibt es folgenden Aufbau (finde ich super!). Kopfzeile (Schul-logo links). Seitenaufbau...Im linken Frame (2 Reiter): 1. Reiter: ist eine Schritt für Schritt Anleitung für die Erarbeitung der Lösung mit kleinen Syntaxhilfen gegeben.2. Reiter: Informationen/Grundlagen/Erklärungen um die Theorie hinter den Aufgabenstellungen zu verstehen (Fundamentals). Im Hauptfenster(rechts daneben) sind die Code-In-Boxen  für die Dateien gegeben die modifiziert/erweitert oder erstellt werden müssen. Falls hier mehrere Dateien bearbeitet werden müssen ist auch hier eine Reiter-Struktur gegeben, sodass jede Datei lt. Aufgabenstellung ergänzt/verändert/erstellt werden kann. Damit die Schüler wissen wo und an welcher Stelle Codesequenzen benötigt werden, helfen ggf. (didaktisch sinnvolle) Kommentare bei der Lösungsfindung. Wenn der Schüler den Schritt  lt. Aufgabenstellung durchgeführt hat kann er auf weiter/solve oder ggf. check klicken um weiter im Kurs fortscheiten zu können. Die Antworten für Theoretische Fragestellungen (Begrifflichkeiten oder ähnliches) können via Multible Choice im Kurs abgefragt werden (Besser als offen!). Wichtig: Das Web-Interface (der Kurs) soll vollständig responsive sein und wenn sinnvoll/möglich auch auf mobilen Endgeräten durchgeführt werden können. Analysiere, prüfe (best practice)und gegebe mir eine Einschätzung/Deine Meinung zum Umfang und Umsetzung.
