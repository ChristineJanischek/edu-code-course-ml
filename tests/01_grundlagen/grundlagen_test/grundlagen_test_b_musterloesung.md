# Musterloesung: Grundlagen-Test – Version B (KI/ML und Python)
...
| Konzept | Kurze Erklaerung | Beispiel aus Alltag oder Industrie |
|---|---|---|
| Kuenstliche Intelligenz | Oberbegriff fuer intelligente Maschinen, die Probleme loesen und Entscheidungen treffen. | Autonome Fahrzeuge navigieren eigenstaendig im Strassenverkehr. |
| Machine Learning | Teilgebiet der KI: Systeme lernen Muster aus Trainingsdaten statt vorherzuprogrammierter Regeln. | Empfehlungssystem, das Filme basierend auf Zuschauer-Verhalten vorschlaegt. |
...
Punktevergabe:
- Pro Zeile 2 Punkte: 1 Punkt Erklaerung, 1 Punkt passendes Beispiel.
...
1. Fest programmierte Systeme folgen vorgegebenen Regeln und veraendern sich nicht. Lernende Systeme passen sich basierend auf neuen Daten an und verbessern ihre Vorhersagen oder Entscheidungen ueber die Zeit. Programmierte Systeme sind ueberall gleich, lernende Systeme werdan durch ihre Daten "geformt".
2. Fest programmiert: Taschenrechner berechnet 2+3 immer gleich. Lernend: Spam-Filter passt sich an neue Spam-Muster an.

Punktevergabe:
- 2 Punkte: Unterschied klar erklaert (Regeln vs. Lernen).
...
1. Die Aufteilung prueft, ob das Modell wirklich generalisieren kann und nicht nur die Trainingsbeispiele "auswendig lernt". Mit separaten Testdaten kann man objektiv bewerten, wie gut das Modell auf neuen, unbekannten Auto-Daten funktioniert.

2. labeled Data: Auto-Datensaetze mit Eingaben (Alter, KM, PS) UND bekanntem Zielwert (aktueller Preis). unlabeled Data: Auto-Informationen ohne bekannte Preise – das Modell koennte diese spaeter vorhersagen.

3. Wenn Testdaten bei Training "mitgelernt" werden, entsteht ein Informationsleck: Das Modell kennt die richtige Antwort bereits. Die Testergebnisse waeren viel zu positiv und unrealistisch, sodass man nicht weiss, wie gut das Modell wirklich ist.

Punktevergabe:
- 2 Punkte: Gute Begruendung (Pruefung auf Generaliserung).
...
- Mit nur 25 Datensaetzen ist das Modell anfaellig fuer Ueberanpassung und Unstaebilkeit. Mit 500 Datensaetzen erfasst das Modell wahrscheinlich mehr Variation in Auto-Preisen und generalisiert besser.
- Fehlerhafte KM-Angaben (z. B. vertauschte Dezimalstellen) verzerren Vorhersagen. Schlechte Datenqualitaet kann MSE erhoehen und R2 verringern, selbst bei grosser Datenmenge.
...
- 1 Punkt: Einfluss der Qualitaet (Fehler → schlechtere Vorhersagen).
...
- Strukturierter Pseudocode wird vollstaendig anerkannt, wenn alle Schritte vorhanden sind.
